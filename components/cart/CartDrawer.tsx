    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.div
            className="fixed inset-x-0 bottom-0 z-50 bg-cream-50 rounded-t-2xl max-h-[85vh] overflow-y-auto"
          ...
          >
            <div className="sticky top-0 bg-cream-50 border-b border-border px-4 py-3 flex items-center justify-between">
              <h2 id="cart-drawer-title" className="text-base font-semibold uppercase tracking-wide text-brown-900">Keranjang</h2>
              <button onClick={onClose} className="p-1 text-brown-500" aria-label="Tutup keranjang"><X className="w-5 h-5" /></button>
            </div>
              ...
              {items.length === 0 ? (
                ...
                <>
                  {items.map((item, i) => (
                    <div key={item.menu_item.id + '-' + i} className="flex items-start justify-between py-3 border-b border-border last:border-0">
                      <div className="flex-1 min-w-0 pr-3">
                        <p className="font-semibold text-[15px] uppercase text-brown-900 leading-snug">{item.menu_item.name}</p>
                        <p className="text-sm font-medium text-ember-600 mt-0.5">{formatCurrency(item.menu_item.price)}</p>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="w-7 h-7 rounded-full border border-border flex items-center justify-center text-brown-900 active:scale-95 transition"
                          aria-label="Kurangi"
                        >
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="w-6 text-center text-sm font-semibold tabular-nums text-brown-900">{item.quantity}</span>
                        <button
                  ...
                  <div className="border-t border-border pt-3">
                    <div className="flex justify-between text-lg font-bold mb-3 text-brown-900">
                      <span>Total</span>
                    ...
                    {tableChosen ? (
                      <p className="flex items-center gap-1.5 text-sm text-[color:var(--color-text-secondary)] mb-3">
                        ...
                        Diantar ke <strong className="text-brown-900">Meja {tableNumber}</strong>
                      </p>
                    ) : (
                      <div className="flex items-start gap-2 rounded-lg bg-gold-100 text-brown-900 px-3 py-2.5 mb-3">
